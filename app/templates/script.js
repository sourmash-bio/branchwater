...
const columns = Object.keys(jsonData[0]).map((key) => {
  const isNumericColumn = jsonData.every((row) => !isNaN(parseFloat(row[key]))); // check if all values in the column are numeric
...
for (let i = 0; i < values.length; i++) {
  if (
    values[i].every((val) => typeof val === "string" && commonKeys[i] !== "acc")
  ) {
...
table.on("dataFiltered", function (filters, rows) {
  ...
  for (let i = 0; i < Filtvalues.length; i++) {
    if (FiltcommonKeys[i] === "geo_loc_name_country_calc") {
      ...
      // fix syntax
      var arr = Filtvalues[FiltcommonKeys.indexOf("lat_lon")];
